function [depos, erosionMap, deposMap, flow, nextFlowXco, nextFlowYco, transRouteXYZ, velo_data, X_data, Y_data, elev_data, grad_data, sed_data, apex_data, obsc_data, conc_data] = calcSteepestDescentTransportAndErosionIB(glob, depos, flow, topog) 
...
% If all deeper destination cells are equal elevation, use a random number to select one - see details in findDeepestNewCell
    ...
    gradient_inner = zeros(iterationLimit,1);
    sedVol_inner = zeros(iterationLimit,1);
    apex_inner = zeros(iterationLimit,1);
    obsc_inner = zeros(iterationLimit,1);
    sedConc_inner = zeros(iterationLimit,1);
    ...
    while flow.stopped == false && iterations < iterationLimit
        ...
        gradient_inner(iterations+1,1) = gradient;
        sedVol_inner(iterations+1,1) = flow.sedVolume;
        apex_inner(iterations+1,1) = apexType;
        obsc_inner(iterations+1,1) = obstacleRelief;
        sedConc_inner(iterations+1,1) = flow.sedConcentration;
    ...
    grad_data = gradient_inner;
    sed_data = sedVol_inner;
    apex_data = apex_inner;
    obsc_data = obsc_inner;
    conc_data = sedConc_inner;
